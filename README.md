...
Однако у нас не получится сравнить полученные данные с данными из эксперимента, поскольку была исследована метка H3F3A. Данные графики (ngs.plot) показывают распределение сигнала метки из ENCODE эксперимента относительно генов. Можно заметить, что данная метка, в основном, расположена в конце последовательности гена, поскольку на графике можно увидеть пик на позиции TES. TES же обозначает конец последовательности гена.
